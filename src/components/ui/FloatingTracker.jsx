...
export default function FloatingTracker() {
  ...
  // Safely format the order ID so it never crashes
  const displayId = activeOrderId 
    ? String(activeOrderId).includes('-') 
      ? String(activeOrderId).split('-')[1] 
      : activeOrderId 
    : "";

  return (
    <AnimatePresence>
      {activeOrderId && !isHidden && (
        <motion.div 
        ...
        >
          ...
          <div className="relative z-10 flex items-center gap-2">
            <div className="text-right">
              ...
              <p className="text-xs font-extrabold">#{displayId}</p>
            </div>
