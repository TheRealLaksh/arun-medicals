            ...
            return (
              ...
              >
                ...
                <Link to={`/product/${item.id}`} className="mt-1 mb-2">
                  ...
                  <p className="text-[10px] text-gray-500 font-medium">{lowestVariant.dosage}</p>
                </Link>

                <div className="mt-auto pt-2 flex items-center justify-between border-t border-gray-100 dark:border-slate-700/50">
                  <div>
                    <p className="font-extrabold text-sm text-primary">₹{discountedPrice.toFixed(2)}</p>
                    {lowestVariant.discount > 0 && <p className="text-[9px] line-through text-gray-400">₹{lowestVariant.price}</p>}
                  </div>
